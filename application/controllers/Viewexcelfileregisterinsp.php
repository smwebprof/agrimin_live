...
class Viewexcelfileregisterinsp extends CI_Controller {
  ...
  public function index()
  {
    ...
    if (!empty($_GET['file_from_date'])|| !empty($_GET['file_To_date'])) {
        $objPHPExcel->getActiveSheet()->SetCellValue('D3', 'File(Inspection) Report for the period "'.$_GET['file_from_date'].'" to "'.$_GET['file_To_date'].'"'); 
      } else { 
        $objPHPExcel->getActiveSheet()->SetCellValue('D3', 'File(Inspection) Report for the Selected All period'); 
      }
     ...
     $objPHPExcel->getActiveSheet()->SetCellValue('H'.$k,'Warehouse');
     $objPHPExcel->getActiveSheet()->SetCellValue('I'.$k,'Status');
     //$objPHPExcel->getActiveSheet()->SetCellValue('J'.$k,'Status');

     $objPHPExcel->getActiveSheet()->getStyle('A'.$k.':I'.$k)->getFont()->setSize(12);
     $objPHPExcel->getActiveSheet()->getStyle('A'.$k.':I'.$k)->getFont()->setBold(true);
    ...
    foreach ($result as $file_data) {
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('H'.$k,$file_data['warehouse']);
        $objPHPExcel->getActiveSheet()->SetCellValue('I'.$k,$file_data['status']);
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A4:I'.($k-1))->applyFromArray($BStyle);
    $objPHPExcel->getActiveSheet()->getStyle('A4:I'.($k-1))->getAlignment()->setWrapText(true); 
    $objPHPExcel->getActiveSheet()->getStyle('A4:I'.($k-1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);

    //echo $pdf1;exit;
    $filename = "File(Inspection)_Report_". date("Y-m-d-H-i-s").".xlsx";   // csv
    ...
    if (!empty($_GET['file_from_date'])|| !empty($_GET['file_To_date'])) {
        $objPHPExcel->getActiveSheet()->SetCellValue('C3', 'File(Inspection) Report for the period "'.$_GET['file_from_date'].'" to "'.$_GET['file_To_date'].'"'); 
      } else { 
        $objPHPExcel->getActiveSheet()->SetCellValue('C3', 'File(Inspection) Report for the Selected All period'); 
      } 
    ...
    //echo $pdf1;exit;
    $filename = "File(Inspection)_Report_". date("Y-m-d-H-i-s").".csv";   // csv
    header('Content-Type: application/vnd.ms-excel'); 
    header('Content-Disposition: attachment;filename="'.$filename.'"');
    header('Cache-Control: max-age=0'); 
    $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'CSV');  
  ...
  public function test()
  {
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('E2', 'AgriMin Control International');
    ...
    if (!empty($_GET['file_from_date'])|| !empty($_GET['file_To_date'])) {
     ...
     $objPHPExcel->getActiveSheet()->SetCellValue('C'.$k,'Sr.No.');
     $objPHPExcel->getActiveSheet()->SetCellValue('D'.$k,'File No');
     $objPHPExcel->getActiveSheet()->SetCellValue('E'.$k,'File Date');
     $objPHPExcel->getActiveSheet()->SetCellValue('F'.$k,'Client Name');
     $objPHPExcel->getActiveSheet()->SetCellValue('G'.$k,'Nomination Date');
     $objPHPExcel->getActiveSheet()->SetCellValue('H'.$k,'Type Of Job');
     $objPHPExcel->getActiveSheet()->SetCellValue('I'.$k,'Scope of Service');
     $objPHPExcel->getActiveSheet()->SetCellValue('J'.$k,'Warehouse');
     $objPHPExcel->getActiveSheet()->SetCellValue('K'.$k,'Status');
     //$objPHPExcel->getActiveSheet()->SetCellValue('J'.$k,'Status');

     $objPHPExcel->getActiveSheet()->getStyle('C'.$k.':K'.$k)->getFont()->setSize(11);
     $objPHPExcel->getActiveSheet()->getStyle('C'.$k.':K'.$k)->getFont()->setBold(true);

     $k=$k+1;
 
    foreach ($result as $file_data) {

        $objPHPExcel->getActiveSheet()->SetCellValue('C'.$k,$k-4);
        $objPHPExcel->getActiveSheet()->SetCellValue('D'.$k,$file_data['file_no']);

        $objPHPExcel->getActiveSheet()->SetCellValue('E'.$k,date("d-m-Y", strtotime($file_data['file_creation_date'])));
        $objPHPExcel->getActiveSheet()->SetCellValue('F'.$k,$file_data['client_name']);
        $objPHPExcel->getActiveSheet()->SetCellValue('G'.$k,date("d-m-Y", strtotime($file_data['nomination_date'])));
        $objPHPExcel->getActiveSheet()->SetCellValue('H'.$k,$file_data['import_export']);
        $objPHPExcel->getActiveSheet()->SetCellValue('I'.$k,$file_data['scope_of_services']);
        $objPHPExcel->getActiveSheet()->SetCellValue('J'.$k,$file_data['warehouse']);
        $objPHPExcel->getActiveSheet()->SetCellValue('K'.$k,$file_data['status']);
        //$objPHPExcel->getActiveSheet()->SetCellValue('J'.$k,$file_data['status']);
    ...
    $objPHPExcel->getActiveSheet()->getStyle('C2:K'.($k-1))->applyFromArray($BStyle);  

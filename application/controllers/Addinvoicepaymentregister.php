...
class Addinvoicepaymentregister extends CI_Controller {
	...
	public function index()
	{
        ...
        if ($_POST) {
        	...
        	if (@$_POST['invoice_balane_amt']<0) {	
                $redirect_url = BASE_PATH."Addinvoicepaymentregister?id=".base64_encode($id)."&msg=1";
        	...
        	if ($resultdata) {
        		...
        		if (!empty($_POST['other_charges'])) { 
        			$invoice_recd_amt = (float)$_POST['invoice_recd_amt'] + (float)$_POST['invoice_rec_amt']+ (float)$_POST['other_charges'];	
        		} else { 
        			$invoice_recd_amt = (float)$_POST['invoice_recd_amt'] + (float)$_POST['invoice_rec_amt'];
        		}

        		//if ($_POST['invoice_amt']==$invoice_recd_amt) {
                if ($_POST['invoice_balane_amt']=='0.00') { 

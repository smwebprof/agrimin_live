...
class Addcargomaster extends CI_Controller {
	...
	public function index()
	{
        ...
        $result = $this->Cargo_Group_master->getCargoGroup();
		...
		if (@$_POST) {
        	...
        	$resultdata = $this->cargo_master->addCargomaster($this->input->post());

			########################### Log Activity ######################################
            $this->load->model('Activity_master');
            $params['module'] = 'addcargomaster';
            $params['date_time'] = $dt;
        	...
        	if ($resultdata) {  
				...
				$this->data['layout_body']='addcargomaster';

...
class Viewexcelinvoicefileregister extends CI_Controller {
  ...
  public function index()
  {
    ...
    if (!empty($_GET['file_from_date'])|| !empty($_GET['file_To_date'])) {
        $objPHPExcel->getActiveSheet()->SetCellValue('D3', 'Commodity Invoice Report for the period "'.$_GET['file_from_date'].'" to "'.$_GET['file_To_date'].'"'); 
      } else { 
        $objPHPExcel->getActiveSheet()->SetCellValue('D3', 'Commodity Invoice Report for the Selected All period'); 
      }
     ...
     $result = $this->Invoice_master->getAllInvoicedataSearch($_GET);
     //print_r($result);exit;
     $tot_inv_amt = 0;
    foreach($result as $key=>$value)
     ...
     $objPHPExcel->getActiveSheet()->SetCellValue('C'.$k,'File Type');
     $objPHPExcel->getActiveSheet()->SetCellValue('D'.$k,'Invoice No');
     $objPHPExcel->getActiveSheet()->SetCellValue('E'.$k,'Invoice Info');
     $objPHPExcel->getActiveSheet()->SetCellValue('F'.$k,'Invoice Date');
     $objPHPExcel->getActiveSheet()->SetCellValue('G'.$k,'Invoice Type');
     $objPHPExcel->getActiveSheet()->SetCellValue('H'.$k,'Invoice Currency');
     $objPHPExcel->getActiveSheet()->SetCellValue('I'.$k,'Invoice Amount');
    ...
    foreach ($result as $invoice_data) {
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('B'.$k,$invoice_data['file_no']);

        $objPHPExcel->getActiveSheet()->SetCellValue('C'.$k,$invoice_data['file_no_type']);
        $objPHPExcel->getActiveSheet()->SetCellValue('D'.$k,$invoice_data['invoice_no']);
        $objPHPExcel->getActiveSheet()->SetCellValue('E'.$k,$invoice_data['invoice_info']);
        $objPHPExcel->getActiveSheet()->SetCellValue('F'.$k,$invoice_data['invoice_date']);
        $objPHPExcel->getActiveSheet()->SetCellValue('G'.$k,$invoice_data['invoice_type']);
        $objPHPExcel->getActiveSheet()->SetCellValue('H'.$k,$invoice_data['currency']);
        $objPHPExcel->getActiveSheet()->SetCellValue('I'.$k,$invoice_data['invoice_amt']);
        $objPHPExcel->getActiveSheet()->SetCellValue('J'.$k,$invoice_data['status']);

        $k++;
    }  

    $objPHPExcel->getActiveSheet()->SetCellValue('H'.$k,'Total Amount');
    $objPHPExcel->getActiveSheet()->SetCellValue('I'.$k,@$tot_inv_amt);
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A4:J'.($k))->applyFromArray($BStyle);
    $objPHPExcel->getActiveSheet()->getStyle('A4:J'.($k))->getAlignment()->setWrapText(true);
    $objPHPExcel->getActiveSheet()->getStyle('A4:J'.($k))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);

    //echo $pdf1;exit;
    $filename = "Commodity_Invoice_Report_". date("Y-m-d-H-i-s").".xlsx";
    ...
    if (!empty($_GET['file_from_date'])|| !empty($_GET['file_To_date'])) {
        $objPHPExcel->getActiveSheet()->SetCellValue('C3', 'Commodity Invoice Report for the period "'.$_GET['file_from_date'].'" to "'.$_GET['file_To_date'].'"'); 
      } else { 
        $objPHPExcel->getActiveSheet()->SetCellValue('C3', 'Commodity Invoice Report for the Selected All period'); 
      } 
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('I'.$k,@$tot_inv_amt);
      	...
      	//echo $pdf1;exit;
    $filename = "Commodity_Invoice_Report_". date("Y-m-d-H-i-s").".csv";
    header('Content-Type: application/vnd.ms-excel'); 
    header('Content-Disposition: attachment;filename="'.$filename.'"');
    header('Cache-Control: max-age=0'); 

...
class Editaccountledgerfileno extends CI_Controller {
	...
	public function index()
	{
        ...
        if (@$_POST) {
        	...
        	if ($_POST['ledger_amount'] > $result[0]['balance_amount']) {
                $redirect_url = BASE_PATH."Editaccountledgerfileno?id=".base64_encode($id)."&msg=1";
	        ...
	        if ($resultdata) {
	        	$redirecturl = BASE_PATH."Viewaccountledgerfileno?msg=1";
	            redirect($redirecturl); 
	        }	

        } else {
        	$this->data['title'] = 'ACI - Editaccountledgerfileno';
			#$this -> load -> model('campaign_model');
			
			$this->data['layout_body']='editaccountledgerfileno';

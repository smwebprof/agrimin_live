...
class Editproformainvoice extends CI_Controller {
        ...
        if ($_POST) {
        	...
        	#echo count($_POST['div_work_type']);exit;
             if (strtotime($_POST['inspection_end_date']) < strtotime($_POST['inspection_start_date'])) {
                $redirect_url = BASE_PATH."Editproformainvoice?id=".base64_encode($id)."&msg=1";
                redirect($redirect_url);
            }

            if (empty($_POST['cargo'][0])) { 
                $redirect_url = BASE_PATH."Editproformainvoice?msg=2";
                redirect($redirect_url);    
            }
        	...
        	$resultdata = $this->Invoice_master->updateEditProformaInvoiceData($this->input->post()); 
            ...
            //$resultdata = 111;
        	//$_POST['profinvoice_no'] = $resultdata;
        	$invoicedata = $this->Invoice_master->updateEditProformaInvoiceDetails($this->input->post());
            //exit;
            ...
            $this->data['id'] = $id;
			$this->data['layout_body']='editperformainvoice'; 

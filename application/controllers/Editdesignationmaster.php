...
class Editdesignationmaster extends CI_Controller {
	...
	public function index()
	{
        ...
        $result = $this->designation_master->getDesignationById($id);
		...
		if (@$_POST) {
        	...
        	$resultdata = $this->designation_master->updateDesignationData($this->input->post());

        	########################### Log Activity ######################################
            $this->load->model('Activity_master');
            $params['module'] = 'editdesignationmaster';
            $params['date_time'] = $dt;
        	...
        	if ($resultdata) {  
				...
				$module = BASE_PATH."viewdesignationmaster/";
                redirect($module);
			...
			$this->data['layout_body']='editdesignationmaster';

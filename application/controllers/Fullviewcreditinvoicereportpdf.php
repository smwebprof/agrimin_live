...
class Fullviewcreditinvoicereportpdf extends CI_Controller {
    ...
    // add library of Pdf
    $this->load->library('PdfCreditenote');
    }
    ...
    public function index()
    {
        ...
        $result = $this->Credit_master->getCreditFileInvoicedata($id);
        //print_r($result);exit;

        $credit_details = $this->Credit_master->getCreditdetailsById($id);
        //print_r($credit_details);exit;
        
        $approx_unit = @$credit_details[0]['approx_unit'];
        ...
        $file_title = 'Credit_Note_'.$result[0]['credit_note_no'];
        $file_name = 'Credit_Note_'.$result[0]['credit_note_no'].'.pdf';
        ...
        foreach($rows as $invoice_data){
          ...
          <tr>
           ...
           $pdf1 .= '<td width="50%">
                  <table width="100%" border="0" align="right">
                      <tr><td><b>Credit Note No : '.$invoice_data['credit_note_no'].'</b></td></tr>
                      <tr><td><b>Credit Note Date : '.date('d-m-Y',strtotime($invoice_data['entry_date'])).'</b></td></tr>
                  </table>   
              </td>';
          $pdf1 .= '</tr>
          </table>';
          ...
          if (!empty($invoice_data['credit_remarks'])) { $pdf3 .= '<tr><th><b>Remarks</b></th><th>'.$invoice_data['credit_remarks'].'</th></tr>'; }

          
          $pdf4 = '<br>';
          //$x = 1;
          $pdf4 .= '<table border="1" cellspacing="0" cellpadding="5" align="center">';
          $pdf4 .=  '<tr><td width="55%"><b>Credit Note Details</b></td><td width="15%"><b>Quantity</b></td><td width="15%"><b>Rate ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].')</b></td><td width="15%"><b>Amount ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].')</b></td></tr>';
          foreach($credit_details as $creditdetails){
                    if ($creditdetails["approx_unit"]=='NONE') { $creditdetails["approx_unit"] = ''; }
                    if (empty($creditdetails["approx_qty"])) { $creditdetails["approx_unit"] = ''; }
                    if ($creditdetails["invoice_work_rate"]==0) { $creditdetails["invoice_work_rate"] = ''; } 
                    $pdf4 .=  '<tr><td align="left">'.htmlspecialchars($creditdetails["work_type"]).'</td><td>'.$creditdetails["approx_qty"].' '.$creditdetails["approx_unit"].'</td>
                    <td>'.$creditdetails["invoice_work_rate"].'</td><td>'.$creditdetails["invoice_work_amount"].'</td></tr>';
                    //$x++;    
          }
          $invoice_ex_amt = $invoice_data["credit_note_basic_amt"]*$invoice_data["credit_note_ex_rate"];
          if (empty($invoice_data["credit_note_tax_amt"])) { $credit_note_tax_amt = ''; } else { $credit_note_tax_amt = $invoice_data["credit_note_tax_amt"]; }
          $pdf4 .=  '<tr><td align="left"><b>Basic Amount ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].'):</b></td><td></td><td></td><td><b>'.$invoice_data["credit_note_basic_amt"].'</b></td></tr>';
          if (!empty($credit_note_tax_amt)) {
          $pdf4 .=  '<tr><td align="left"><b>Tax Amount (VAT '.$invoice_data["credit_note_vat_percent"].'%) :</b></td><td></td><td></td><td><b>'.$credit_note_tax_amt.'</b></td></tr>'; }
          $pdf4 .=  '<tr><td align="left"><b>Total Amount ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].'):</b></td><td></td><td></td><td><b>'.sprintf('%.2f', $invoice_data["credit_note_amt"]).'</b></td></tr>';
          $pdf4 .= '<table border="1" cellspacing="0" cellpadding="5" align="center">';
                          $pdf4 .=  '<tr><td width="100%"><b>Amount : - </b> '.$invoice_data["currency"].' '.strtoupper($this->numtowords($invoice_data["credit_note_amt"],$invoice_data["subunit"])).'</td></tr>';
                          $pdf4 .=  '</table>';
          $pdf4 .= '</table>';
        ...
        $pdf = new PdfCreditenote(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
          ...
          $content = '<br><br><br>';
          $content .= '<h3 align="center"><u>Credit Note</u></h3><br /><br />';
          ...
          $content .= '<h3 align="center"><u>Credit Note Details</u></h3><br /><br />';
          #$content .= '<br>';
          $content .= $pdf4;
    ...
    public function index_prev0502()
    {
        ...
        //print_r($result);exit;
        if (($result[0]['tax_options']=='VAT') && ($result[0]['credit_note_tax_amt']=='')) {
        ...
        foreach($rows as $invoice_data){
          ...
          if (count($cargo_group_arr)>1) { 
            if ($cargo_discount_flag==0) { 
              $pdf4 .=  '<tr><td><b>Cargo Group</b></td><td width="30%"><b>Credit Note Details</b></td><td width="15%"><b>Quantity</b></td><td width="15%"><b>Rate ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].')</b></td><td width="20%"><b>Amount ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].')</b></td></tr>';
            } else { 
                $pdf4 .=  '<tr><td><b>Cargo Group</b></td><td width="30%"><b>Credit Note Details</b></td><td width="11%"><b>Quantity</b></td><td width="11%"><b>Rate ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].')</b></td><td width="11%"><b>Discount (%)</b></td><td width="20%"><b>Amount ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].')</b></td></tr>';
            }  
          } else {   
             if ($cargo_discount_flag==0) {
                $pdf4 .=  '<tr><td width="50%"><b>Credit Note Details</b></td><td width="15%"><b>Quantity</b></td><td width="15%"><b>Rate ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].')</b></td><td width="20%"><b>Amount ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].')</b></td></tr>';
             } else {
                $pdf4 .=  '<tr><td width="35%"><b>Credit Note Details</b></td><td width="15%"><b>Quantity</b></td><td width="15%"><b>Rate ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].')</b></td><td width="15%"><b>Discount (%)</b></td><td width="20%"><b>Amount ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].')</b></td></tr>';
             } 
            ...
            <td>'.$invoicedetails["invoice_work_rate"].'</td><td>'.$invoicedetails["invoice_work_amount"].'</td></tr>';*/
              ...
              if (count(@$cargo_group_arr)>1) {  
                if ($cargo_discount_flag==0) { 
                   $invoice_ex_amt = $invoice_data["credit_note_basic_amt"]*$invoice_data["credit_note_ex_rate"];
                    if (empty($invoice_data["credit_note_tax_amt"])) { $credit_note_tax_amt = ''; } else { $credit_note_tax_amt = $invoice_data["credit_note_tax_amt"]; }
                    $pdf4 .=  '<tr><td align="left"><b>Basic Amount ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].'):</b></td><td></td><td></td><td></td><td><b>'.$invoice_data["credit_note_basic_amt"].'</b></td></tr>';
                    ...
                    if (!empty($credit_note_tax_amt)) {
                    $pdf4 .=  '<tr><td align="left"><b>Tax Amount (VAT '.$invoice_data["credit_note_vat_percent"].'%) :</b></td><td></td><td></td><td></td><td><b>'.$credit_note_tax_amt.'</b></td></tr>';
                    }
                    $pdf4 .=  '<tr><td align="left"><b>Total Amount ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].'):</b></td><td></td><td></td><td></td><td><b>'.sprintf('%.2f', $invoice_data["credit_note_amt"]).'</b></td></tr>';
                    $pdf4 .=  '</table>';
                    $pdf4 .= '<table border="1" cellspacing="0" cellpadding="5" align="center">';
                    #$pdf4 .=  '<tr><td><b>Conversion Rate : </b> '.$invoice_data["credit_note_ex_rate"].' '.$invoice_data["currency"].' <b>Amount : </b>'.$invoice_ex_amt.'</td></tr>';
                    $pdf4 .=  '</table>';
                    $pdf4 .= '<table border="1" cellspacing="0" cellpadding="5" align="center">';
                    $pdf4 .=  '<tr><td><b>Amount : - </b> '.$invoice_data["currency"].' '.strtoupper($this->numtowords($invoice_data["credit_note_amt"],$invoice_data["subunit"])).'</td></tr>';
                    $pdf4 .=  '</table>';     
                } else {  
                   $invoice_ex_amt = $invoice_data["credit_note_basic_amt"]*$invoice_data["credit_note_ex_rate"];
                    if (empty($invoice_data["credit_note_tax_amt"])) { $credit_note_tax_amt = ''; } else { $credit_note_tax_amt = $invoice_data["credit_note_tax_amt"]; }
                    $pdf4 .=  '<tr><td align="left"><b>Basic Amount ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].'):</b></td><td></td><td></td><td></td><td></td><td><b>'.$invoice_data["credit_note_basic_amt"].'</b></td></tr>';
                    if (!empty($invoice_data["invoice_discount"])) {
                    ...
                    if (!empty($credit_note_tax_amt)) {
                    $pdf4 .=  '<tr><td align="left"><b>Tax Amount (VAT '.$invoice_data["credit_note_vat_percent"].'%) :</b></td><td></td><td></td><td></td><td></td><td><b>'.$credit_note_tax_amt.'</b></td></tr>';
                    }
                    $pdf4 .=  '<tr><td align="left"><b>Total Amount ('.$invoice_data["sortname"].' '.$invoice_data["symbol"].'):</b></td><td></td><td></td><td></td><td></td><td><b>'.sprintf('%.2f', $invoice_data["credit_note_amt"]).'</b></td></tr>';
                    $pdf4 .=  '</table>';
                    $pdf4 .= '<table border="1" cellspacing="0" cellpadding="5" align="center">';
                    #$pdf4 .=  '<tr><td><b>Conversion Rate : </b> '.$invoice_data["credit_note_ex_rate"].' '.$invoice_data["currency"].' <b>Amount : </b>'.$invoice_ex_amt.'</td></tr>';
                    $pdf4 .=  '</table>';
       ...
       #$logo = ASSETS_PATH.'img/agrilogo269x74.jpg';
        ...
        foreach($rows as $invoice_data){
          ...
          foreach($row_invoice as $invoicedetails){  
           ...
           #echo $this->numtowords($invoice_data["credit_note_amt"]);exit;    
          ...
          /*$pdf4 .= '<table border="1" cellspacing="0" cellpadding="5">
            ...
            $pdf4 .= '<td></td>';
            $pdf4 .= '<td></td>';
            $pdf4 .= '<td>'.$invoice_data["credit_note_tax_amt"].'</td>'; 
            $pdf4 .= '</tr>';
            ...
            $pdf4 .= '<td></td>';
            $pdf4 .= '<td></td>';
            $pdf4 .= '<td>'.$invoice_data["credit_note_amt"].'</td>'; 
            $pdf4 .= '</tr>'; 

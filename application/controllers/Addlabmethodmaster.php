...
class Addlabmethodmaster extends CI_Controller {
	...
	public function index()
	{
        ...
        if (@$_POST) {
            ...
            $getlabmethods = $this->Lab_master->getBylabMethods($this->input->post());
            if ($getlabmethods) {
            	$redirecturl = BASE_PATH."Addlabmethodmaster?msg=1";
                redirect($redirecturl);
            } else {
            	$resultdata = $this->Lab_master->addlabmethods($this->input->post());
            }

			########################### Log Activity ###################################
            $this->load->model('Activity_master');
            $params['module'] = 'addlabmethodmaster';
            $params['date_time'] = $dt;
        	...
        	if ($resultdata) {
        		...
        		$redirecturl = BASE_PATH."Viewlabmethodmaster?msg=1";
                redirect($redirecturl);
        	...
        	$branchs = $this->Branch_master->getBranchdataByStatus();
        	#print_r($branchs);exit;

   			$this->data['title'] = 'ACI - Addlabmethodmaster';
			#$this -> load -> model('campaign_model');
			
			$this->data['layout_body']='addlabmethodmaster';

...
class Viewexcelfileregister extends CI_Controller {
  ...
  public function index()
  {
    ...
    if (!empty($_GET['file_from_date'])|| !empty($_GET['file_To_date'])) {
        $objPHPExcel->getActiveSheet()->SetCellValue('D3', 'File(Single) Report for the period "'.$_GET['file_from_date'].'" to "'.$_GET['file_To_date'].'"'); 
      } else { 
        $objPHPExcel->getActiveSheet()->SetCellValue('D3', 'File(Single) Report for the Selected All period'); 
      }
    ...
    //echo $pdf1;exit;
    $filename = "File(Single)_Report_". date("Y-m-d-H-i-s").".xlsx";   // csv
    ...
    if (!empty($_GET['file_from_date'])|| !empty($_GET['file_To_date'])) {
        $objPHPExcel->getActiveSheet()->SetCellValue('C3', 'File(Single) Report for the period "'.$_GET['file_from_date'].'" to "'.$_GET['file_To_date'].'"'); 
      } else { 
        $objPHPExcel->getActiveSheet()->SetCellValue('C3', 'File(Single) Report for the Selected All period'); 
      } 
    ...
    //echo $pdf1;exit;
    $filename = "File(Single)_Report_". date("Y-m-d-H-i-s").".csv";
    header('Content-Type: application/vnd.ms-excel'); 
    header('Content-Disposition: attachment;filename="'.$filename.'"');
    header('Cache-Control: max-age=0'); 
    $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'CSV');  

...
class Addaccountledgerfileno extends CI_Controller {
	...
	public function index()
	{
        ...
        $files_data = $this->Vendor_master->getAllFiledata();
        ...
        if (@$_POST) {
	        ...
	        $params['file_no'] = $_POST['ledger_file_no'];
	        $params['invoice_no'] = $_POST['ledger_invoice_no'];
	        $params['invoice_amt'] = $_POST['ledger_invoice_amt'];
	        //print_r($params);exit;
	        ...
	        $this->data['title'] = 'ACI - Addaccountledgerfileno';
			#$this -> load -> model('campaign_model');
			
			$this->data['layout_body']='addaccountledgerfileno';
			...
			if ($_POST['ledger_file_no']) {
				$this->data['post_ledger_file_no'] = $_POST['ledger_file_no'];
			...
			$this->data['vendor_details']=$vendor_details;
			//$this->data['invoice_data'] = $invoice_data;
			$this->data['files_data'] = $files_data;
        	...
        	if (!empty($_POST['vendor_name'])) {
        		...
        		if (!empty($vendor_details)) {
        			if ($_POST['ledger_type']=='Credit') {
	        			...
	        			if (!empty($vendor_closingbl[0]['credit_amount']))  {
	        				$params1['balance_amount'] = $vendor_closingbl[0]['credit_amount']+$_POST['ledger_amount'];
	        			} else {
	        			...
	        			if (!empty($vendor_closingbl[0]['debit_amount']))  {
        			...
        			if ($_POST['ledger_type']=='Debit') {
	        			...
	        			$params1['ledger_amount'] = $_POST['ledger_amount'];
	        			if (!empty($vendor_closingbl[0]['debit_amount']))  {
	        				$params1['balance_amount'] = $vendor_closingbl[0]['debit_amount']-$_POST['ledger_amount'];
	        			} else {
	        				$params1['balance_amount'] = $_POST['ledger_amount'];
	        			}
	        			...
	        			if (!empty($vendor_closingbl[0]['credit_amount']))  {
	        				$params['balance_amount'] = $vendor_closingbl[0]['credit_amount']+$_POST['ledger_amount'];
	        			} else {
	        				$params['balance_amount'] = $vendor_closingbl[0]['balance_amount']+$_POST['ledger_amount'];
	        			}
	...
	public function fetch_inv_amt()
	{
		...
		echo $this ->Vendor_master->getInvoiceDetailsByInvno($this->input->post('ledger_invoice_no'));

	}

	public function fetch_inv_byfileno()
	{
		$this->load->model('Vendor_master'); 
		
		echo $this ->Vendor_master->fetch_inv_byfileno($this->input->post('ledger_file_no'));

...
class Editcargogroupmaster extends CI_Controller {
	...
	public function index()
	{
        ...
        $result = $this->Cargo_Group_master->getCargoGroupById($id);
		...
		if (@$_POST) {
        	...
        	$resultdata = $this->Cargo_Group_master->updateCargoGroupData($this->input->post());

        	########################### Log Activity ######################################
            $this->load->model('Activity_master');
            $params['module'] = 'editcargogroupmaster';
            $params['date_time'] = $dt;
        	...
        	if ($resultdata) {  
				...
				$module = BASE_PATH."viewcargogroupmaster/";
                redirect($module);
 
            }

		} else {
			$this->data['title'] = 'ACI - Cargo Group Master';
			$this->data['cargo_group_data'] = $result;
			$this->data['layout_body']='editcargogroupmaster';

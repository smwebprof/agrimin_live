...
class Addmaildocsmaster extends CI_Controller {
	...
	public function index()
	{
		...
		if (@$_POST) {
        	...
        	$resultdata = $this->Document_master->addMailDocsmaster($this->input->post());

        	########################### Log Activity ######################################
            $this->load->model('Activity_master');
            $params['module'] = 'addmaildocsmaster';
            $params['date_time'] = $dt;
        	...
        	if ($resultdata) {  
				...
				$this->data['layout_body']='addmaildocsmaster';

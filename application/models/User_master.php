...
class User_master extends CI_Model{ 
    ...
    function getRows($params){ 
        ...
        $querystring = "SELECT * FROM agrimin_employee_users_master WHERE office_email = '".$params['useremail']."' and password = '".$params['password']."' and is_active = 1";
   ...
   public function updateUserDetails($data)
    ...
    //print_r($result);exit;
    $this->db->insert('agrimin_login_master',$result);
    return $this->db->insert_id();
    ...
    if(empty($data))
      return FALSE;
       ...
       //print_r($result);exit;
        ...
        $this->db->update('agrimin_login_master',$result);
        #echo $this->db->last_query();exit;
        return (($this->db->affected_rows() > 0)?TRUE:FALSE);
   ...
   function getAllLogindata($data){ 
        ...
        $querystring = 'SELECT alm.*,aeum.first_name,aeum.last_name,abm.branch_name FROM agrimin_login_master alm left join agrimin_employee_users_master aeum ON alm.user_id=aeum.id left join agrimin_branch_master abm ON alm.branch_id=abm.id Where date(alm.login_date) >= "'.date('Y-m-d',strtotime($data['login_to_date'])).'" and date(alm.login_date) <= "'.date('Y-m-d',strtotime($data['login_from_date'])).'" and aeum.id not in (11,12,13) Order by alm.id desc';
    ...
    function getAllLogindataSearch($data){ 
        ...
        $querystring = "SELECT alm.*,aeum.first_name,aeum.last_name,abm.branch_name FROM agrimin_login_master alm left join agrimin_employee_users_master aeum ON alm.user_id=aeum.id left join agrimin_branch_master abm ON alm.branch_id=abm.id Where alm.is_active = 1 and aeum.id not in (11,12,13) $search  Order by alm.id desc";
    ...
    function getAllLoginEmp(){ 
        ...
        $querystring = "select aeum.emp_code,aeum.company_id,acm.name company_name,aeum.branch_id,abm.branch_name,aeum.id,aeum.first_name,aeum.middle_name,aeum.last_name,aeum.office_email,aeum.personal_email,aeum.password from agrimin_employee_users_master aeum left join agrimin_company_master acm on aeum.company_id=acm.id left join agrimin_branch_master abm on aeum.branch_id=abm.id Where aeum.id not in (11,12,13) order by aeum.id";
        $queryforpubid = $this->db->query($querystring);

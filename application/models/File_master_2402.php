...
class File_master extends CI_Model{ 
  ...
  function fetch_cargo($params)
   ...
   function getAllFiledata(){  //$id
        ...
        $querystring =  "SELECT aft.*,acm.client_name,accm.commodity_name,aum.unit_name, aiem.name import_export FROM agrimin_fileregister_transaction aft left join agrimin_client_master acm ON acm.id=aft.client_id left join agrimin_cargo_master accm ON accm.id=aft.cargo_id left join agrimin_unit_master aum ON aum.id=aft.approx_unit left join agrimin_import_export_master aiem ON aiem.id=aft.import_export Where aft.is_active = 1 and aft.user_comp_id = '".$_SESSION['comp_id']."' and aft.user_branch_id = '".$_SESSION['branch_id']."' OR aft.invoice_by_branch = '".$_SESSION['branch_id']."' OR find_in_set('".$_SESSION['userId']."',aft.user_id) order by aft.id desc";

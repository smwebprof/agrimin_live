...
<script>
        ...
        jQuery(document).ready(function() {       
           ...
           Custom.initCreditnoteregister();
           ComponentsDropdowns.init();
           //FormFileUpload.init();
           TableAdvanced.init();


           /*var status_id = $('#credit_status').val();

           if (status_id=='Cancelled') {
              $(".credit_cancel_remarks1").show();
           } 

           if (status_id=='Open') {
              $(".credit_cancel_remarks").hide();
           }*/

           /*$(".credit_cancel_remarks").show();
           $(".credit_cancel_remarks1").hide();*/
        ...
        function other_rows_mult(i) { 
            var inv_tbody1 = $('#invoice_cargo_item_details_'+i).children('tbody');
            //Then if no tbody just select your table 
            var inv_table1 = inv_tbody1.length ? inv_tbody1 : $('#invoice_cargo_item_details_'+i);

            inv_table1.append('<tr class="active"><td><input type="text" class="form-control input-large" placeholder="" name="invitems_cargo_name_oth[]" id="invitems_cargo_name" value="" ><input type="hidden" class="form-control input-large" placeholder="" name="invitems_cargo_details_oth[]" id="invitems_cargo_details" value="invitems_cargo_details_oth|'+i+'" ><input type="hidden" name="invitems_cargo_group_oth[]" id="invitems_cargo_group" value="Other"></td><td><input type="text" class="form-control input-small" placeholder="" name="invitems_quantity_oth[]" id="invitems_quantity" value="" onkeyup="cargo_amount4(this);"></td><td><select class="form-control input-small select2me" data-placeholder="Select..." name="invitems_unit_oth[]" id="invitems_unit"><option value=""></option><option value="%">%</option><option value="KG">KG</option><option value="LTRS">LTRS</option><option value="MT">MT</option><option value="MMT">MMT</option><option value="NOS">NOS</option><option value="PER BAG">PER BAG</option><option value="PER CONTAINER">PER CONTAINER</option><option value="PER DAY">PER DAY</option><option value="PER DAY/SURVEYOR">PER DAY/SURVEYOR</option><option value="PER HEAP">PER HEAP</option><option value="PER HOLD">PER HOLD</option><option value="PER LOT">PER LOT</option><option value="PER MAN">PER MAN</option><option value="PER MMT">PER MMT</option><option value="PER MONTH">PER MONTH</option><option value="PER MT">PER MT</option><option value="PER PERSON">PER PERSON</option><option value="PER PILE">PER PILE</option><option value="PER PLOTE">PER PLOTE</option><option value="PER RAKE">PER RAKE</option><option value="PER SAMPLE">PER SAMPLE</option><option value="PER SHIFT">PER SHIFT</option><option value="PER STACK">PER STACK</option><option value="PER SURVEY">PER SURVEY</option><option value="PER TANKER">PER TANKER</option><option value="PER UNIT">PER UNIT</option><option value="NONE">NONE</option></select></td><td><input type="text" class="form-control input-small" placeholder="" name="invitems_rate_oth[]" id="invitems_rate" value="0" onkeyup="cargo_amount4(this);"></td><td width="10%"><input type="text" class="form-control input-small invitems_amt" placeholder="" name="invitems_amt_oth[]" id="invitems_amt" value=""></td></tr>');
...
/*function selectAll(){
      ...
      $('#credit_status').change(function(){
             //$(".credit_cancel_remarks").prop('required',true);
             var status_id = $('#credit_status').val();

             if (status_id=='Cancelled') { 
                $(".credit_remarks").prop('required',true);
             } 

             if (status_id=='Open') {
                $(".credit_remarks").prop('required',false);
             }
        });

<!-- BEGIN PAGE HEADER-->
			<div class="row">
				<div class="col-md-12">
					...
					<h3 class="page-title">
					View Weight Quality Certificates
					</h3>
				...
				<div class="col-md-12">
					...
					<div class="portlet box grey">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-user"></i>View Weight Quality Certificates
							</div>
							<div class="actions">
								<?php /*
						...
						<div class="portlet-body">
						    ...
						    <tr class="odd gradeX">
								...
								<td class="center">
     								<?php echo @$this->data['cert_data'][$invoice_data['id']][$invoice_data['cert_id']]['status']; ?>
								</td>				
								<?php if ($this->data['access_right']['add_rights'] == 1) { ?>
								<td>
								<?php if (@$this->data['invoice_details'][$invoice_data['file_no']] != 'Final') { ?>
									  <?php if (@$this->data['cert_data'][$invoice_data['id']][$invoice_data['cert_id']]['status'] != 'Final') { ?>
									  <?php if (@$this->data['cert_data'][$invoice_data['id']][$invoice_data['cert_id']]['status']=='Draft') { ?>
									  <span class="label label-sm label-info">
											<a href="<?php echo BASE_PATH; ?>editweightqualitycertificate?id=<?php echo base64_encode(@$this->data['cert_data'][$invoice_data['id']][$invoice_data['cert_id']]['cert_id']); ?>"  style="color:#fff">Edit</a>
										</span>
									  <?php } else { ?>	
									  <span class="label label-sm label-success">
											<a href="<?php echo BASE_PATH; ?>addweightqualitycertificate?id=<?php echo base64_encode($invoice_data['id']); ?>"  style="color:#fff">Add New</a>
									  </span>
									  <?php } ?>
									  <?php } ?>
							    <?php } ?>
								...
								<td>
									 <?php if (@$this->data['cert_data'][$invoice_data['id']][$invoice_data['cert_id']]['status'] == 'Final') { ?>
									 <span class="label label-sm label-info">
										<a href="<?php echo BASE_PATH; ?>Downloadweightqualitycertificate?id=<?php echo base64_encode(@$this->data['cert_data'][$invoice_data['id']][$invoice_data['cert_id']]['cert_id']); ?>"  style="color:#fff" target="_blank"> Download (PDF Format)</a>
									 </span>
									 <?php if ($_SESSION['country_code']=='CH') { ?>
									 &nbsp;&nbsp;/&nbsp;&nbsp; 
									 <span class="label label-sm label-success">
										<a href="<?php echo BASE_PATH; ?>Downloadweightqualitycertificateword/weight_quality_cert_form?id=<?php echo base64_encode(@$this->data['cert_data'][$invoice_data['id']][$invoice_data['cert_id']]['cert_id']); ?>"  style="color:#fff" target="_blank"> Download Word (Word Format)</a>
									 </span><?php } ?>					<?php } ?> 
								</td>
								<?php /*
								<?php if ($this->data['access_right']['edit_rights'] == 1) { ?>
